...
const Companylogo = () => {
    const logos = [
        slack,amazon,meundies,sitepoint,woocommerce
    ]
  return (
    <div className="w-full overflow-hidden container mx-auto py-20 gap-8 flex sm:flex-row flex-col sm:items-center items-start px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex whitespace-nowrap animate-marquee ">
        {logos.map((logo, index) => (
            <img key={'duplicate-${index}'} src={logo} alt="company logos" className="mx-12 h-8 w-36 obj-contain grayscale opacity-70 
            custom-hover"/>
        ))}

        {/* duplicate logo */}
        {logos.map((logo, index) => (
            <img src={logo} alt="company logos" className="mx-12 h-8 w-36 obj-contain grayscale opacity-70 
            custom-hover"/>
        ))}
